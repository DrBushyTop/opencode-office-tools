...
export default excel("manage_named_range", "Create, update, rename, set visibility, or delete workbook-scoped Excel named ranges.", {
  action: tool.schema.enum(["create", "update", "rename", "setVisibility", "delete"]).describe("Named range operation to perform."),
  name: tool.schema.string().describe("Existing or new workbook-scoped named range name depending on the action."),
  newName: tool.schema.string().optional().describe("New named range name for rename."),
  reference: tool.schema.string().optional().describe("Cell or formula reference for a workbook-scoped named range, such as A1:D10, Sheet1!B2, or =SUM(A:A)."),
